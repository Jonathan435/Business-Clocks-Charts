...
<br/>Please note:<br/>
1.  It is recomended not to change font because the getter of this property is calling ```LoadFont()``` function in order to calculate the suitable font em size and style relative to the clock size using internal elgoritem.
2. If the clock is to small the text will not be shown inside the clock (when not changing ```ClockFont``` from outside).
